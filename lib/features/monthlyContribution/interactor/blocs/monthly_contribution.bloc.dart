...
class MonthlyContributinoBloc
  ...
  _create(MonthlyContributionEventCreate event, emit) async {
    try {
      ...
      emit(const MonthlyContributionStateError(
          error: "Contribuição mensal não cadastrada."));
    }
  }

  _update(MonthlyContributionEventUpdate event, emit) async {
    try {
      emit(MonthlyContributionStateLoading());
      await datasource.update(event.monthlyContribution);
    } catch (e) {
      emit(const MonthlyContributionStateError(
